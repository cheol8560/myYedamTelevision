...
function loadCommentList() {
	...
	new ajax.xhr.Request("/CommentController", param, loadReplyResult, 'GET');
}
//목록조회 콜백함수
...
function makeCommentView(comment){
	...
	div.setAttribute("id", "c"+comments.commentNo);
	div.className = 'comment';
	div.comment = comment;  //{id:1,.... }
	
	var s="";
	if(reply.writer == "${login}"){
		s = "<input class=\"button\" type=\"button\" value=\"수정\" onclick=\"viewUpdateForm('"+comments.commentNO+"')\"/>"
		+"<input class=\"button\" type=\"button\" value=\"삭제\" onclick=\"confirmDeletion('"+comments.commentNO+"')\"/>"
	}
	var str = "<strong>" + comments.memberId +" : "+ "</strong>" + comments.commentText +"<span class=\"date\">"+ comments.writeDate+"</span>" 
				+s
...
function addComment() {
	...
	var memberId = '${login}';//document.addForm.writer.value;
	...
	new ajax.xhr.Request('/CommentController', params, addResult, 'POST');
}
////댓글 등록 콜백함수
...
function updateComment() {
	...
	new ajax.xhr.Request('/CommentController', params, updateResult, 'POST');
}
	...
	if (req.readyState == 4) {
		if (req.status == 200) {
			...
			if (code == 'success') {
				...
				var oldCommentDiv = document.getElementById('c'+comments.commentNO);
...
function viewUpdateForm(commentNo) {
	var commentsDiv = document.getElementById('c'+commentNo);
	...
	document.updateForm.commentNo.value = comments.commentNo;    
	document.updateForm.memberId.value = comments.memberId;
	document.updateForm.commentText.value = comments.commentText;
...
function confirmDeletion(commentNo) {
	if(confirm("삭제할까요?")){
		...
		new ajax.xhr.Request('/CommentController', params, deleteResult, 'POST');
	}
	...
	if (req.readyState == 4) {
		if (req.status == 200) {
			...
			if (code == 'success') {
				...
				var commentDiv = document.getElementById('c'+comments.commentNo);
